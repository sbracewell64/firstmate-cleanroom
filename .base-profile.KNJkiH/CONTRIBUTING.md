...
- Helper scripts in `bin/` are plain bash.
  ...
  A missing or wrong-version linter is reported as one typed `ENVIRONMENT_UNREADY:` line naming the owner action, never as a lint verdict, and the gate fails rather than skipping the check.
  `bin/fm-tool-profile.sh` is the single owner of the qualified tool-profile observation (pinned shellcheck and actionlint, tasks-axi floor and exact CI pin, no-mistakes floor, and a Node that actually executes `.ts`, which the Pi extension tests import through plain `node`); run it from the shell you are unsure about, including `bash -lc`, and CI's `tool-profile` job runs it from a direct, an `sh -c`, and a login shell plus a legacy-shaped login-shell negative control so a lost tool selection is caught before a gate pays for it.
  Workflow YAML checks use `bin/fm-workflow-yaml.sh`, and the same parser capability is required by the tool profile before validation.
  Its probe records the selected backend, library version and executable from the actual gate shell; provision missing capability through that environment's dependency owner rather than an ad hoc system install.
...
- An enforce, validate or refuse capability in `bin/` is declared in [`docs/enforcement-points.json`](docs/enforcement-points.json) with the production call site that reaches it; run `bin/fm-enforcement-caller-check.sh` after adding or rewiring one, because a capability that only tests call is not enforcing anything.
  That check confirms the declared call site still names the capability in executable text; it does not prove the shell runs it, so wiring the call correctly stays the author's job.
...
The captain launcher/console is versioned as a host-agnostic source at [`bin/enter-firstmate.sh`](bin/enter-firstmate.sh); its header and `--doctor` own the mechanics, including the six-profile primary console menu (fable-5.1 built-in default, opus-4-8, codex-luna, pi-sol, pi-astra, pi-luna-max), the `$FM_HOME/config` host-path resolution, and the zero-dollar/subscription launch boundary, where a Pi grant binds its exact versioned `openai-codex` route and the live Pi owner checks catalog, OAuth, effort and endpoint before launch.
...
Pi profile activation remains PENDING until a supported account owner proves zero usable purchased credits and disabled automatic reload, or an equivalent control preventing purchased-credit use; OAuth availability and a successful request alone do not prove this spend posture.
...
The caller's Herdr pane-ownership refusal and lifecycle isolation are pinned by `tests/test_console_lifecycle.py`, with the dedicated-pane positive path covered by `tests/test_startup_selection.py`.
