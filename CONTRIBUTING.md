...
The captain launcher/console is versioned as a host-agnostic source at [`bin/enter-firstmate.sh`](bin/enter-firstmate.sh); its header and `--doctor` own the mechanics, including the three-profile primary console menu (codex-luna default, fable-5.1, opus-4-8), the `$FM_HOME/config` host-path resolution, and the zero-dollar/subscription launch boundary.
