...
for shape in live malformed symlink hardlink fifo; do
  ...
  printf "reconcile with %s runner: %s\n" "$shape" "$orphan_out"
...
orphan_out=$(FM_HOME="$H_ORPHAN" "$PROCEVENT" reconcile 2>&1)
printf 'reconcile with dead runner: %s\n' "$orphan_out"
...
printf "Captured extension result:\n"
cat "$H_ORPHAN/state/procevent-inbox/orphan-source.1.result"
printf "\nDurable watcher queue:\n"
wait_for_file "$H_ORPHAN/state/.wake-queue" || fail "captured orphan result was never queued"
cat "$H_ORPHAN/state/.wake-queue"
FM_HOME="$H_ORPHAN" "$PROCEVENT" handled orphan-source 1
printf "Acknowledged source status:\n"
FM_HOME="$H_ORPHAN" "$PROCEVENT" list
...
printf "Immediate reservation count: %s (expected 2)\n" "$override_crash_records"
find "$STATE_OVERRIDE/procevent-capture-reservations" -type f -printf "%f\n"
