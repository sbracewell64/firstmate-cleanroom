...
- Slices A, B, and D bind no qualification record yet: their steps name the path where the qualification owner's record must land, and until it does the resolver reports `REQUIRED_BINDING_MISSING` for the first of them, never completion.
  ...
  A local delivery qualifies only after the programme pins its owner, record digest, generation, policy, exact candidate head/tree/delivery identity, and the resolver reproduces its private receipt, separated maker/checker evidence, git candidate, digest-only privacy declaration, manifest, and destination read-back.
