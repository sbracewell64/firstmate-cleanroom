...
The lifecycle stage transitions belong to `bin/fm-stage.sh`, which calls this owner's `launch`, `bind`, and `refresh` and never writes the obligation itself; the per-run two-level assessment is a later increment of the same programme and is not part of this owner.
