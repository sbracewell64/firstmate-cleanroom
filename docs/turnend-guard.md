...
A legacy build's lock-holding claim (recognizable by its `autoarm` role file) still defers or reclaims under the legacy abandonment proof: an identity-verified live stuck owner receives the one recorded TERM this shim ever delivers, keeps its exact lock, and may be collected only by a later invocation that freshly observes it gone, while an owner whose recorded identity is absent, unreadable, or no longer matched by its live pid is never signalled and is reclaimed as-is, an identity that changes between recording that attempt and sending the signal withdraws the record instead of stalling the owner, and a failed reclaim re-blocks rather than allowing a blind stop.
That record lives inside the owner's own lock as owner evidence, so a collection that fails retains or restores it and the single allowance is never respent on a lock that survives.
An upgrade mid-session can therefore never double-arm, but the deferral is deliberately not deadlock-free: an old-build owner that survives its one TERM holds its lock on every later pass, so the guard keeps blocking turn ends instead of spending its attended fail-open alarm until that owner is killed - the safe direction, and the trade `fm_autoarm_release_abandoned` in `bin/fm-wake-lib.sh` owns.
