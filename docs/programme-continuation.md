...
- The `local_project_delivery` owner is the privacy-preserving route for a private/local exact candidate: the public programme record carries identities and digests only, while a pinned mode-0600 receipt under the operational home binds one project whose registered posture is `local-only`, exact branch head and tree, distinct maker and checker receipts, policy, generation, manifest, and checker-attributed read-back.
