...
4. The remaining qualification record for D, owner the qualification and landing owners: when that slice is independently qualified, its owner record is authored at the step's `terminal_predicate.evidence` path as an `fm-accepted-owner-evidence/v1` record.
