...
test_network_phase_partitions_the_run() {
  local case_dir fakebin isolated_path tool all_out skip_out only_out combined
  case_dir="$TMP_ROOT/network-phase"
  mkdir -p "$case_dir/home/config"
  printf '%s\n' manual > "$case_dir/home/config/backlog-backend"
  fakebin=$(make_fake_toolchain "$case_dir")
  # Keep this fixture independent of whether the host image happens to ship
  # node. The case intentionally removes node from its fake toolchain; an
  # ambient /bin/node must not make that negative input disappear.
  isolated_path="$case_dir/path"
  mkdir -p "$isolated_path"
  for tool in /bin/*; do
    [ "${tool##*/}" = node ] && continue
    ln -s "$tool" "$isolated_path/${tool##*/}"
  done
...
SH
  ...
  all_out=$(PATH="$fakebin:$isolated_path" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
    FM_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/fm-bootstrap.sh")
  ...
  skip_out=$(PATH="$fakebin:$isolated_path" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
  ...
  only_out=$(PATH="$fakebin:$isolated_path" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
  ...
  [ "$(PATH="$fakebin:$isolated_path" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
