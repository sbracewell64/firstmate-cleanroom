...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without a holding validation wait and without their own task worktree being written, declared external waits, declared validation waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
A pane whose crew is in a declared validation wait that still holds is deferred instead of escalated, because such a crew is idle by contract: the pipeline drives the run, the worker waits for the next gate, and its status log gets no new entry for the whole run.
A wait is declared by the lifecycle stage receipt `bin/fm-stage.sh` writes when the real run is bound to the candidate, and it holds only while the run's own `active_steps` activity is younger than `FM_PIPELINE_ACTIVITY_MAX_SECS`, so no worker prose is needed to quiet the pane and no declaration alone can quiet it.
An activity age no-mistakes has marked `quiet` still counts against that same bound, because a quiet step is one the pipeline is still tracking whose log has gone silent rather than one it has given up on, and the marker is carried through so a deferral names the quiet reading it actually held on instead of claiming fresh output.
Both halves are required, and every negative outcome leaves the existing escalation schedule untouched: a run that stopped progressing, ended, failed, or was never declared keeps escalating exactly as before.
That deferral re-surfaces on the `FM_PAUSE_RESURFACE_SECS` cadence with a reason naming the validation wait rather than a wedge, so a run that quietly stops making real progress cannot stay invisible.
...
Continuation authority for a pinned programme step is never folded from status prose or held rows: `bin/fm-continuation-resolve.sh` is the one typed owner of the next action and its authority, and the session-start digest, fleet snapshot, bearings, fleet view, and away-mode digest embed its result verbatim ([`programme-continuation.md`](programme-continuation.md)).
`bin/fm-programme-projection.sh` composes that same result into a stateless projection tuple with phase, applicability, and returned-not-enforced delegation bounds, and holds no state or authority of its own.
The resolver's completion-evidence seam reads proof dispositions and one closed owner-evidence record kind, and its clock-free material identity is what `bin/fm-programme-presentation-lib.sh` keys the wake drain's present-once, quiet-when-unchanged, ack-what-was-presented behavior on.
...
Away-mode housekeeping has neither the worktree-write deferral nor the declared-validation-wait deferral of its own, so while `state/.afk` exists a quiet crew that is writing its own worktree, or waiting on a running validation, still escalates as a possible wedge at that bound.
...
The typed worker-discipline selection is compiled and rendered by `bin/fm-work-context-discipline-lib.sh` through the existing work-context, while `bin/fm-work-context-engineering-lib.sh` composes its checked receipt with skill and verification context; [configuration.md](configuration.md#engineering-work-context) owns the resulting behavior and identity contract.
That contract hands the worker the lifecycle stage commands of `bin/fm-stage.sh`, the one owner of the ship task's stage sequence (candidate committed, validation pending or admitted, validation running, an optional candidate successor, CI-ready, landing, activated), of the stage fields it records in the task record, and of the transition receipt each stage line carries; the worker never writes a stage line by hand, and duplicate delivery of a transition is a no-op.
For a no-mistakes task the `committed` command issues the validation transition itself under the task's standing authority, admitting the launch through `bin/fm-nm-observe.sh` and refusing into an explicit wait when a hold is open or capacity is missing, so validation starts without a conversational steer; `bin/fm-classify-lib.sh` classifies each stage verb exactly one way and `bin/fm-crew-state.sh` renders a recorded stage as current state when no run outranks it.
...
`bin/fm-project-mode.sh` remains the registry parser for mechanical consumers that have no task in hand, while governed local artifact admission parses one retained `data/projects.md` snapshot through `bin/fm-local-project-delivery.py`.
The admission manifest seals the digest of that complete registry snapshot, so any later registry change requires fresh admission even when the changed row is unrelated to the selected project.
When a selected delivery path calls for a diff, `bin/fm-review-diff.sh` refreshes the authoritative base and, when task meta records `pr=`, always fetches and compares against the live forge pull head by default (`refs/pull/<n>/head` on GitHub, `refs/merge-requests/<n>/head` on GitLab; recorded `pr_head=` is only an offline fallback) before falling back to the local branch with a warning.
Either fallback is labelled on stdout as a could-not-observe result, both sides are pinned to exact SHAs in the header, and malformed meta refuses rather than degrading; the script header owns those contracts.
