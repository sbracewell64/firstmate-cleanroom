...
The watcher takes that lock and holds it across the marker mutation and the singleton-lock release, so the uninterruptible region contains no wait at all: the transition it calls is the already-held variant and performs no acquire of its own.
...
The stop disposition in force during the wait releases the lock before exiting, so a stop taken at any point of it leaves no lock held by a dead pid.
