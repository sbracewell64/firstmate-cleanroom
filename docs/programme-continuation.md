...
- The programme file is validated as a machine boundary at load, and the resolver header owns the exact rules: a parse failure propagates the parser's own error, each step is an object with a slug id and a closed evidence kind, each dependency entry and each accepted outcome is exactly one non-empty identifier with no whitespace, and every defect is refused naming the step and the offending entry rather than being split, dropped, or folded into an empty value.
...
- The `local_project_delivery` owner is the privacy-preserving route for a private/local exact candidate: before dispatch or a delivery effect, `bin/fm-local-project-delivery.py bind` derives one immutable private admission from the programme action's declared family, an exact source root, and a strictly registered `local-only` destination project, then binds source bytes to the destination's branch head, tree, tracked paths, Git objects, and file modes with distinct maker/checker requirements and one qualification route.
- The programme step's `terminal_predicate.local_delivery` object is the family policy owner, including any pinned destination project and preservation identities; a family with no complete registered owner remains CNO and a wrong or ambiguous owner is refused without publication.
- The step's `terminal_predicate.local_delivery_source` object is the public source-root identity owner: `canonical_artifact_root` admits only a source root outside the home's registered project tree, `owner_project_root` admits only the delivery owner project's own root as an authorized same-root delivery, and a step that declares neither cannot bind or qualify. The pin names no operator-local path, so which root may qualify a delivery is published rather than self-declared by the private admission.
- A V2 delivery receipt binds that mode-0600 admission by digest before the resolver accepts checker-attributed read-back; the public programme record still carries identities and digests only, and the resolver re-runs the admission verifier against current source bytes, project registration, ref, candidate objects, modes, family, and preservation identities on every resolution.
  That verification consumes the exact source-root identity sealed in each immutable admission rather than the programme root or ambient working directory, and refuses any sealed root the step's declared source identity does not admit, while every admission still has to match the current complete registry digest, so a changed registry invalidates old records and mixed-generation A/B/D qualification cannot pass.
- A moved head or tree, stale or replayed admission, owner or family mismatch, self-certification, privacy exposure, wrong mode, read-back mismatch, or unavailable input is refused or CNO and never qualification; the V1 receipt remains readable for already-produced PR #64 evidence but does not bypass V2 admission for new records.
- A programme with any owner-evidence step must carry a `binding` object naming the commission, the controlling grant, the consumer contract, the supported evidence kinds, and the programme generation; the resolver verifies it against its own contract and schema and refuses a missing member or a mismatch at load, and a legacy proof-only programme without one still resolves while every reader prints `REQUIRED_BINDING_MISSING` rather than an optional N/A.
...
Every result carries a clock-free `material_identity`: a digest over the programme and binding identities, the next action and its generation, the classification, authority state, reason code, CNO reason code, accountable owner, predecessor, current, and evidence identities, gating holds, answered facts, and materialize axes, with the date, the CNO detail text, and every path excluded, so the same material state in a home at another absolute path yields the same identity.
...
A later no-ack turn that observes the same identity as a lingering pending record promotes that record too, so the readers converge on unchanged instead of pending-ack, and it still promotes only the identity that was presented.
...
The same library owns resolver stream capture for all five consumers: typed stdout and diagnostic stderr remain separate, diagnostics stay visible, and the signal cleanup of whichever process actually staged the capture removes the shared staging record without a sourced-library trap replacing its caller's traps.
The programme projection and the session-start digest capture in their main shell and clean up through their own top-level traps; the wake drain, the fleet snapshot, and the away-supervisor digest capture inside a subshell, so each of those subshells arms the shared cleanup itself, because its parent can neither see nor remove a staging file that never leaves the child.
Exit 3 (no programme configured) remains silent on both streams for every consumer, so an unconfigured home is not narrated on each drain, digest, snapshot, and session start.
The observed interrupted-staging leak is attributed to the earlier per-caller cleanup gap.
The earlier hosted schema-failure shape is only consistent with merged diagnostics; no emitting process was identified, so its origin remains CNO and the deterministic noisy-stderr regression proves the boundary rather than claiming that incident cause.
...
The programme binds the controlling captain grant and the completed architecture ruling by source, represents the Proof-B `CNO_AT_B-S9` transition through that ruling's own record bound to the adverse disposition by exact bytes, and binds the landed slices to their governed owner records; a slice whose qualification record is not yet bound resolves as `REQUIRED_BINDING_MISSING`, never as complete.
Its A, B, and D predicates now admit the governed local-project delivery outcome without weakening the existing forge or ruling routes, but no outcome is projected until each slice's exact owner-produced record is pinned.
...
The same suite proves the owner-evidence adapter: real accepted A-E records under the grant yield the pilot, a landing or self-report without its qualification cannot, every mismatch and forged receipt is refused CNO, the Proof-B transition is accepted only through the ruling record bound to the exact adverse bytes and outcome, structure and binding defects are refused at load (including a dependency or accept entry that is empty, whitespace-only, multi-token, or non-string, a non-object step, and an unparseable programme file, each named in the refusal, while every valid dependency spelling of one sequence resolves to the same typed result), applicability and material identity move on evidence, grant, and hold changes and ignore the clock, and the wake drain presents once, stays quiet, and acknowledges only the presented identity across the present-then-change-then-ack race.
`tests/fm-programme-projection.test.sh` proves the substrate's authority fields equal the resolver's under every driven classification, that the resolver's load refusal of a malformed dependency entry in the sibling owner-evidence fixture propagates through the projection rather than composing an empty tuple, that each structural change makes a prior applicability tuple non-matching, that two calls over unchanged state are identical and write nothing, that every backlog access is a read, that delegation bounds are returned without a spawn, that an owner-evidence programme composes unchanged with the pilot's concurrency-2 bound returned and nothing launched, and that a record it cannot compose (an unreadable or nested candidate worktree, duplicate step ids, or an off-ladder ceiling) is refused rather than nulled or selected around.
The same suite drives diagnostic separation, caller-trap preservation, and deterministic signal interruption through the executable projection and asserts that shared resolver staging is removed.
