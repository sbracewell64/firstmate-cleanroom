...
It replaces both fields rather than appending them, so a second review with a changed key set corrects the recorded inventory instead of leaving the earlier one shadowed behind it; `bin/fm-backlog-transition-lib.sh` owns that partial-write rule and the single-value-per-key record contract it enforces.
