...
#                   docs/fm-test-portable-shards.md records this table
#   --check-coverage
#                   prove the lane partition (see below) and fail when any
#                   portable serial shard's summed hints exceed
#                   PORTABLE_SERIAL_SHARD_BUDGET_PERCENT of the
#                   PORTABLE_SERIAL_JOB_CAP_MINUTES job cap.
#                   FM_PORTABLE_SERIAL_JOB_CAP_MINUTES=<n> runs the same guard
#                   against a what-if cap without changing the CI job
...
# It also states the shard job cap and budgets each shard's summed duration
# hints against it in --check-coverage, so hint drift fails the guard before a
# shard is cancelled at the cap.
...
# The CI job cap each portable serial shard runs under, in minutes. It mirrors
# `timeout-minutes` on the tests-portable-serial job in .github/workflows/ci.yml,
# and tests/fm-test-run.test.sh refuses when the two disagree. The coverage
# guard budgets every shard's summed hints at PORTABLE_SERIAL_SHARD_BUDGET_PERCENT
# of this cap, so shard drift fails --check-coverage before a runner is
# cancelled at the cap; docs/fm-test-portable-shards.md owns the measured
# spread behind both numbers. FM_PORTABLE_SERIAL_JOB_CAP_MINUTES (set and
# non-empty, whole minutes 1..360 with no leading zero) overrides the cap for a
# what-if check only; it never changes the CI job.
PORTABLE_SERIAL_JOB_CAP_MINUTES=30
PORTABLE_SERIAL_SHARD_BUDGET_PERCENT=66
...
family_for_basename() {
  case "$1" in
    ...
    fm-remote-backlog-handoff-failfast.test.sh|\
    ...
    fm-herdr-version-floor-live-e2e.test.sh|enter-firstmate-pane-isolation-e2e.test.sh|\
...
# that carried it, and a script repaired in the same change as a refresh
# carrying the scaled local estimate that doc records until the next refresh
# measures it. These are balance hints only: the shard partition stays
# complete and disjoint whatever they say, so a stale hint costs balance
# rather than coverage. That doc owns the refresh procedure.
portable_serial_weight_hints() {
  cat <<'EOF'
tests/enter-firstmate-arm.test.sh 75
tests/enter-firstmate-launch.test.sh 134749
tests/enter-firstmate-profile.test.sh 235
tests/enter-firstmate-render.test.sh 157474
tests/fm-afk-inject-e2e.test.sh 34731
tests/fm-afk-pi-herdr-return-e2e.test.sh 94
tests/fm-afk-return.test.sh 1764
tests/fm-ask-user-authority.test.sh 105
tests/fm-backend-cmux-smoke.test.sh 30
tests/fm-backend-cmux.test.sh 3632
tests/fm-backend-herdr-focus-flash-e2e.test.sh 20
tests/fm-backend-orca.test.sh 18665
tests/fm-backend-tmux-smoke.test.sh 305
tests/fm-backend-zellij-smoke.test.sh 22
tests/fm-backend-zellij.test.sh 9075
tests/fm-backend.test.sh 20844
tests/fm-backlog-atomicity.test.sh 120135
tests/fm-backlog-handoff.test.sh 50994
tests/fm-bearings-board-render.test.sh 1336
tests/fm-bearings-board.test.sh 3881
tests/fm-bearings-snapshot.test.sh 78931
tests/fm-bootstrap-network-parallel.test.sh 8751
tests/fm-bootstrap.test.sh 57995
tests/fm-branch-supervision.test.sh 5561
tests/fm-busy-adapter-wiring.test.sh 19702
tests/fm-busy-state.test.sh 2881
tests/fm-calm-pi-extension.test.sh 600
tests/fm-check-unregister.test.sh 446
tests/fm-classify-corr-token.test.sh 16245
tests/fm-classify-decision-key.test.sh 1092
tests/fm-claude-stop-autoarm-live-e2e.test.sh 18
tests/fm-claude-stop-autoarm.test.sh 60621
tests/fm-cmux-claude-composer-live-e2e.test.sh 22
tests/fm-codex-continuity-live-e2e.test.sh 19
tests/fm-commit-identity-verify.test.sh 259
tests/fm-composer-matrix-live-e2e.test.sh 20
tests/fm-continuation-lib.test.sh 241
tests/fm-continuation-resolve.test.sh 142281
tests/fm-control-relaunch.test.sh 44123
tests/fm-control.test.sh 48373
tests/fm-cursor-harness.test.sh 30195
...
tests/fm-herdr-session-cleanup.test.sh 5983
tests/fm-herdr-submit-confirm-live-e2e.test.sh 21
tests/fm-herdr-version-floor-live-e2e.test.sh 21
tests/fm-home-summary-refresh.test.sh 33601
tests/fm-inactive-reconcile.test.sh 41082
tests/fm-kimi-harness.test.sh 19185
tests/fm-lint-workflows.test.sh 740
tests/fm-muse-harness.test.sh 67130
tests/fm-muse-signals-live-e2e.test.sh 24
tests/fm-nm-assess.test.sh 8905
tests/fm-nm-commit-identity.test.sh 1066
tests/fm-nm-observe.test.sh 40068
tests/fm-no-mistakes-required.test.sh 406
tests/fm-on.test.sh 11439
tests/fm-opencode-primary-live-e2e.test.sh 21
tests/fm-operational-input.test.sh 746
tests/fm-outbound-write.test.sh 6504
tests/fm-peek-remote.test.sh 965
tests/fm-pending-reply.test.sh 24471
tests/fm-pi-branch-extension.test.sh 20370
tests/fm-pi-branch-live-e2e.test.sh 20
tests/fm-pi-primary-live-e2e.test.sh 20
tests/fm-pi-watch-extension.test.sh 43280
tests/fm-pr-check-security.test.sh 162423
tests/fm-procevent-quota.test.sh 1911
tests/fm-procevent-when.test.sh 15641
tests/fm-procevent.test.sh 63320
tests/fm-programme-projection.test.sh 25280
tests/fm-project-origin.test.sh 117
tests/fm-public-followup.test.sh 86000
tests/fm-quota-array-dispatch-live-e2e.test.sh 19
tests/fm-quota-choose.test.sh 1343
tests/fm-remote-backlog-handoff-failfast.test.sh 9434
tests/fm-remote-backlog-handoff.test.sh 40578
tests/fm-remote-doctor.test.sh 5214
tests/fm-remote-entrypoint.test.sh 121
tests/fm-remote-job-orphan-reap.test.sh 2985
tests/fm-remote-job.test.sh 63523
tests/fm-remote-reply.test.sh 45881
tests/fm-remote-secondmate-lifecycle-e2e.test.sh 185270
tests/fm-remote-secondmate-parent-binding.test.sh 27108
tests/fm-remote-secondmate-trace-context.test.sh 56457
tests/fm-remote-transport-lanes.test.sh 62751
tests/fm-secondmate-harness.test.sh 150442
tests/fm-secondmate-lifecycle-e2e.test.sh 7693
tests/fm-secondmate-liveness.test.sh 11078
tests/fm-secondmate-reconcile.test.sh 65929
tests/fm-secondmate-safety.test.sh 50586
tests/fm-secondmate-sync.test.sh 16060
tests/fm-send-inbox-doorbell-live-e2e.test.sh 20
tests/fm-send-inbox.test.sh 52867
tests/fm-send-remote-delivery.test.sh 25998
tests/fm-send-resolve-key.test.sh 18778
tests/fm-send-secondmate-marker-herdr-e2e.test.sh 68
tests/fm-send-secondmate-marker.test.sh 5506
tests/fm-session-lock-ancestry.test.sh 1229
tests/fm-session-start.test.sh 160285
tests/fm-sessionstart-hook-live-e2e.test.sh 19
tests/fm-sessionstart-instruction-refresh-live-e2e.test.sh 20
tests/fm-sessionstart-nudge.test.sh 66437
tests/fm-shared-captain-inheritance.test.sh 6143
tests/fm-spawn-codex.test.sh 5906
tests/fm-spawn-commit-identity.test.sh 7179
tests/fm-spawn-dispatch-profile.test.sh 67772
tests/fm-spawn-pool-base-freshen.test.sh 37136
tests/fm-spawn-work-context-gate.test.sh 13777
tests/fm-spawn-worktree-settle.test.sh 5973
tests/fm-stage.test.sh 55994
tests/fm-startup-memory-budget.test.sh 6653
tests/fm-startup-network.test.sh 56981
tests/fm-stow-cascade.test.sh 3013
tests/fm-subagent-pretool-check.test.sh 942
tests/fm-supervision-events.test.sh 635
tests/fm-tangle-guard.test.sh 9853
tests/fm-task-delivery.test.sh 6038
tests/fm-task-inbox.test.sh 24543
tests/fm-teardown-endpoint-safety.test.sh 3966
tests/fm-teardown.test.sh 85320
tests/fm-test-fixture-cleanup.test.sh 801
tests/fm-test-fixtures.test.sh 131
tests/fm-test-isolation-proof.test.sh 2377
tests/fm-tmux-agent-liveness.test.sh 1534
tests/fm-tool-profile.test.sh 2463
tests/fm-tool-update-check.test.sh 13946
tests/fm-trace-context-lib.test.sh 225
tests/fm-trace-context-spawn.test.sh 81365
tests/fm-turnend-guard-epipe.test.sh 832
tests/fm-turnend-guard.test.sh 21235
tests/fm-update.test.sh 5053
tests/fm-vendor-auth-probe.test.sh 43203
tests/fm-voice-relay.test.sh 28803
tests/fm-wake-daemon-lifecycle-e2e.test.sh 7122
tests/fm-wake-drain-open-decisions-cursor.test.sh 17798
tests/fm-wake-drain-open-decisions.test.sh 6346
tests/fm-wake-drain-unread-status.test.sh 13175
tests/fm-wake-queue.test.sh 46924
tests/fm-watch-arm.test.sh 54438
tests/fm-watch-checkpoint.test.sh 5753
tests/fm-watch-recovery-loop.test.sh 58815
tests/fm-watch-triage.test.sh 265573
tests/fm-watcher-lock.test.sh 67292
tests/fm-work-context.test.sh 16530
tests/fm-workflow-yaml.test.sh 577
EOF
}
...
# Summed hint per portable serial shard, printing
# "portable-serial-<k>of<n>\t<script count>\t<hint_ms>" for every shard in
# shard order. --serial-shard-loads prints this for a hint refresh, and the
# coverage guard budgets it against the job cap.
portable_serial_shard_loads() {
  local shard script i
  local -a loads=() counts=()
  i=1
  while [ "$i" -le "$PORTABLE_SERIAL_SHARDS" ]; do
    loads[i]=0
    counts[i]=0
  ...
  while IFS=$'\t' read -r shard script; do
    [ -n "$script" ] || continue
    loads[shard]=$((loads[shard] + $(portable_serial_weight_for "$script")))
    counts[shard]=$((counts[shard] + 1))
  done < <(portable_serial_assignments)
  i=1
  while [ "$i" -le "$PORTABLE_SERIAL_SHARDS" ]; do
    printf 'portable-serial-%sof%s\t%s\t%s\n' "$i" "$PORTABLE_SERIAL_SHARDS" "${counts[i]}" "${loads[i]}"
    i=$((i + 1))
  done
}

# The shard job cap in minutes, honoring the what-if override. An override
# that is set but empty, non-numeric, zero, leading-zero (octal in arithmetic),
# or above GitHub's 360-minute job maximum is refused rather than guessed.
portable_serial_job_cap_minutes() {
  local cap=${FM_PORTABLE_SERIAL_JOB_CAP_MINUTES-$PORTABLE_SERIAL_JOB_CAP_MINUTES}
  case "$cap" in
    ''|*[!0-9]*|0*) die "FM_PORTABLE_SERIAL_JOB_CAP_MINUTES must be a whole number of minutes from 1 to 360 with no leading zero" ;;
  esac
  [ "${#cap}" -le 3 ] && [ "$cap" -le 360 ] \
    || die "FM_PORTABLE_SERIAL_JOB_CAP_MINUTES must be a whole number of minutes from 1 to 360 with no leading zero"
  printf '%s\n' "$cap"
}
...
run_coverage_guard() {
  ...
  # Every shard's summed hints stay inside the budget the job cap allows, so a
  ...
  while IFS=$'\t' read -r lane count load; do
    [ -n "$lane" ] || continue
    [ "$load" -le "$max_load" ] || max_load=$load
    [ "$load" -le "$budget" ] || over="$over $lane=${load}ms/${count}scripts"
  done < <(portable_serial_shard_loads)
  if [ -n "$over" ]; then
    log "coverage guard: portable serial shard hint total exceeds $PORTABLE_SERIAL_SHARD_BUDGET_PERCENT% of the ${cap}-minute job cap (${budget}ms):$over"
    log "refresh the hints and rebalance per docs/fm-test-portable-shards.md before a runner is cancelled at the cap"
    rm -rf "$tmp"
    return 1
  fi
  ...
  printf 'FM_TEST_COVERAGE ok total=%s parallel=%s serial=%s serial_shards=%s herdr=%s serial_cap_min=%s serial_shard_budget_ms=%s serial_shard_max_hint_ms=%s\n' \
    "$(wc -l <"$tmp/all" | tr -d ' ')" \
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-peek.sh|bin/fm-composer*)
      printf '%s\n' backend-dispatch
      printf '%s\n' pure-contract-unit
      # fm-spawn.sh also carries the wired work-context authority gate, the
      # no-mistakes commit-identity pin it calls for a no-mistakes ship spawn, and
      # the per-process worker commit-identity export at the launch seam.
      case "$path" in
        bin/fm-spawn.sh)
          printf '%s\n' "__script__:fm-spawn-work-context-gate.test.sh"
