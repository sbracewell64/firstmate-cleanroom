...
- The `local_project_delivery` owner is the privacy-preserving route for a private/local exact candidate: the public programme record carries identities and digests only, while a pinned mode-0600 receipt under the operational home binds one project whose registered posture is `local-only`, exact branch head and tree, distinct maker and checker identities, policy, generation, manifest, and checker-attributed read-back through its bound checker receipt.
- The resolver independently re-reads the current local branch, candidate blobs, checker receipt, and destination bytes on every resolution; a moved head or tree, replayed or changed receipt, owner or generation mismatch, self-certification, privacy exposure, read-back mismatch, or unavailable input is refused or CNO and never qualification.
