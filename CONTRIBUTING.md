...
- An enforce, validate or refuse capability in `bin/` is declared in [`docs/enforcement-points.json`](docs/enforcement-points.json) with the production call site that reaches it; run `bin/fm-enforcement-caller-check.sh` after adding or rewiring one, because a capability only tests call is not enforcing anything.
