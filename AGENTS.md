...
config/programme     optional pin of the programme whose continuation authority bin/fm-continuation-resolve.sh owns; LOCAL, gitignored, and not inherited; see docs/configuration.md "Programme continuation"
...
state/               runtime records and signals; gitignored
  ...
  <id>.commit-identity  durable commit-identity obligation for a no-mistakes ship task (name=/email= lines); written by fm-spawn and consumed at the CI-ready boundary by bin/fm-pr-check.sh, which for a GitHub ship fails closed - refusing to arm the merge poll with an UNVERIFIED result whenever a required input is missing/unreadable, the base is unavailable, or the verifier exits nonzero (a contaminated pipeline commit included) - while an obligated GitLab task arms without the check because its PR head is structurally unavailable there (bin/fm-commit-identity-verify.sh owns the check)
  ...
  outbound-writes/   private ledger of every outbound write's exact payload bytes, digest, read-back verdict, and provider evidence, including the one undelivered record a rejected write leaves; written only by bin/fm-outbound-write-lib.sh, pruned on the seven-day horizon, never a retry trigger
  ...
  .programme-presented .programme-presented.pending  the programme continuation's acknowledged and pending presented material identities, so unchanged programme state is presented once and never recapped; owned by bin/fm-programme-presentation-lib.sh, safe to delete (re-presents the current state once)
...
3. **Wake queue** - when locked, presents the durable wake queue and prints the raw records prominently as this turn's first work queue; a clearly labeled status-event annotation may follow a valid `signal` record and includes every status line still unread at the presentation cursor, but never replaces the raw record or current-state reconciliation, and a lapsed watcher chain still surfaces here via the same guard alarm.
   ...
   When a programme is pinned, a `PROGRAMME CONTINUATION` section prints only when the typed result's material state changed since it was last presented, and its absence on a later drain means unchanged state to consume silently, never that the programme is gone (`bin/fm-programme-presentation-lib.sh`).
...
5. **Fleet-state digest** - after that read-once contract and ahead of the context digest, the compact backlog listing owned by `bin/fm-session-start.sh`; a `Programme continuation` subsection carrying the typed result of `bin/fm-continuation-resolve.sh` whenever `config/programme` pins a programme (silent when unpinned, failure shown rather than guessed); every `state/<id>.meta`; a bounded tail of each task's `state/<id>.status` (labeled as wake-EVENT history, not current state, with the full log path printed for a deeper read); the `state/.afk` flag; and one cheap alive/dead read of each task's recorded backend endpoint.
...
7. **Context digest and next step** - last of the bulk sections, the full contents of `data/projects.md` and `data/secondmates.md`, then `data/captain.md`, `data/captain-shared.md`, and `data/learnings.md` injected whole when they fit the startup-memory budget and marked `WITHHELD` under a `STARTUP_MEMORY_BUDGET` diagnostic otherwise, each clearly delimited, followed by the closing reminder.
...
Before declaring any task blocked or complete at resume, read its state back against the authoritative child, qualification, and runtime owners rather than trusting stale roadmap or backlog wording; `bin/fm-work-context.sh` composes those existing owners and a landed child already refreshes its roadmap owner at the terminal transition (docs/configuration.md "Work-context authority gate").
...
Load `project-management` before adding, creating, removing, or initializing a project, and before binding a governed local-project artifact delivery.
...
For a pinned programme (`config/programme`), `bin/fm-continuation-resolve.sh` is the single owner of the next action and its authority, and its typed result outranks generic caution, historical prose, and unbound holds inside its exact scope.
Never state or imply that a programme step waits on the captain unless that typed result is CAPTAIN; when it is, materialize the hold through it rather than describing one.
...
3. For `check:`, act on the named poll result, including merges, Relay events, process-to-event source results, and captain inbox notes; a handled inbox note is also acknowledged with `bin/fm-inbox.sh drain --ack <id>`, or it stays counted as still waiting for firstmate.
   A `check: nm-observe` wake means the watcher's non-consuming `reconcile --peek` saw a new or changed coverage finding; run `bin/fm-nm-observe.sh reconcile --now`, which prints the same lines and commits the cursor, and apply each printed heal command.
...
With no active worker, judge selection per task on each item's own holds, dependencies, phase, and authority, never from the aggregate home state: an aggregate `externally_held` or `captain_decision` is not a global hold, so an independent eligible task beside a held one is still selectable, and a queued item is not a standing instruction to dispatch it (`bin/fm-work-context.sh select` composes the existing `eligible_queued` producer with the per-task read-back).
...
`bin/fm-work-context-discipline-lib.sh` compiles typed task facts into the ship/promotion discipline recorded in the existing work-context, renders its shared kernel, and owns the scout evidence subset; the engineering extension of `bin/fm-work-context-lib.sh` binds that selection plus applicable skill sources and proof obligations through fresh/resumed context, stages, and completion.
...
- `project-management` - load before adding, creating, removing, or initializing a project, and before binding a governed local-project artifact delivery.
  Cloning or registering a project is add intake and uses the same trigger.
- `pr-presentation` - load before preparing or regenerating a substantial PR body after implementation, review, or CI changes its candidate; trivial PRs keep their concise path without loading it.
