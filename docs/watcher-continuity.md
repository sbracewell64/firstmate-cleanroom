...
Direct drain callers receive that instruction on stdout.
Session start relays the drain-owned stdout unchanged without parsing, authenticating, or reconstructing the instruction; separately captured resolver diagnostics remain visibly prefixed and never grant acknowledgement authority.
...
Stopping a watcher is CONFIRMED by observing the process gone, never assumed from a queued signal, because the target's shell can consume a trapped signal without running its handler and carry on with its close path unrun.
The arm layer's `--restart` stop and owned-child cleanup deliver one exact-identity TERM and use the strict one-second timeout owner; the legacy auto-arm reclaim and the test suites' `reap` use [`fm_stop_process_confirmed`](../bin/fm-wake-lib.sh).
The legacy auto-arm reclaim leaves a live owner's lock in place even after that stop is confirmed, so the next firing can enter the existing proven-dead reconciliation path; an unreadable live-owner identity or a confirmation window that ends while the owner remains live likewise makes reclamation refuse and leave the lock for a later firing.
That reclaim confirmation ceiling is fixed at one second, preserving the legacy bound rather than widening with an environment override.
