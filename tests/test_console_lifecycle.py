...
class LifecycleTests(unittest.TestCase):
    ...
    def test_live_different_model_refuses(self):
        self.f.record(harness='codex', profile='codex-luna', model='gpt-5.6-sol')
        self.f.inventory([{'workspace_id':'w7','pane_id':'w7:p1'}])
        result = self.f.run()
        self.assertNotEqual(result.returncode, 0, result.stderr)
        self.assertEqual(self.f.effects(), '')
    ...
    def test_creation_forwards_explicit_profile(self):
        ...
        result = self.f.run(FM_CONSOLE_PROFILE='codex-luna')
        self.assertEqual(result.returncode, 0, result.stderr)
        ...
        self.assertEqual(forwarded['FM_CONSOLE_PROFILE'], 'codex-luna')
