...
`bin/fm-project-mode.sh` remains the one registry parser for mechanical consumers that have no task in hand: fleet sync's `local-only` skip, home seeding's refusal and no-mistakes initialization, and strict registered-owner reads by governed local artifact admission.
