...
#   --console-profile <name>  config/console-profile            (default: codex-luna)
...
# all three profiles appear, proving the composed menu renders offline.
scratch=$(mktemp -d)
...
if [ "$menu_ok" = 1 ]; then
  q_pass+=("print-console-menu: renders the three-profile menu")
else
  q_fail+=("print-console-menu: renders the three-profile menu")
fi
rm -rf "$scratch"
...
# --- 4. report -------------------------------------------------------------------
{
  ...
  echo "- Codex primary composition (codex-luna default, gpt-5.6-luna)"
