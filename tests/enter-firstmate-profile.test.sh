...
console_profile_model_ok pi-sol gpt-5.6-sol && fail "a bare id cannot alias subscription Sol"
console_profile_model_ok pi-sol openai-codex/gpt-5.6-sol:max && fail "Sol max cannot alias xhigh"
...
# shellcheck disable=SC2016 # the generated harness must receive a literal positional argument
for _h in claude codex pi; do printf '#!/bin/sh\n[ "${1:-}" != --version ] || echo 0.81.1\nexit 0\n' > "$TMP/bin/$_h"; chmod +x "$TMP/bin/$_h"; done
...
pass "gate: Pi routes stay PENDING despite stale name-only grants"
