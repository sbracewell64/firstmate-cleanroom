...
| `fm-programme-presentation-lib.sh` | Single owner of the programme continuation's quiet presentation: present a material identity once, stay quiet on unchanged state, and acknowledge only the identity actually presented |
...
| `fm-stage.sh`            | One owner of the ship task's lifecycle stage transitions, their receipts, and the code-issued validation admission |
...
| `fm-enforcement-caller-check.sh` | Prove every enforce-style entry point in `bin/` declares a call site that still names it in executable text (docs/verification/enforcing-call-sites.md) |
...
| `fm-project-mode.sh`     | Resolve a project's registered delivery posture, with strict registration for authority readers |
| `fm-local-project-delivery.py` | Bind and re-verify immutable owner-bound admissions for governed local programme deliveries |
...
| `fm-nmf-verify-input.sh` | Typed input mode for the `Require no-mistakes` gate: historical event snapshot for opened/edited; for synchronize/reopened, single owner of the live-subject read, the superseded-subject guard, the bounded same-subject publication wait that only waits while a publication is in flight, the hand-off that leaves every verdict to the pinned verifier, and the post-verdict read-back |
...
| `fm-wake-drain.sh`       | Present and acknowledge the current actor's claimed wake rows alongside status, decision, divergence, programme-continuation, recovery, and supervision checks |
