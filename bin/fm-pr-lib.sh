...
fm_pr_metadata_identity_parse() {
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    case "$line" in
      ...
      stage=*|stage_epoch=*|stage_branch=*|stage_head=*|stage_landed_head=*|stage_landed_head_source=*|stage_tree=*|stage_gen=*|stage_attempt=*|stage_run=*|stage_pr=*|stage_reason=*|stage_context=*|stage_evidence=*)
        ;;
