...
#
#   state: <working|parked|done|blocked|paused|failed|unknown> · source: <run-step|pane|status-log|remote-endpoint|none> [· activity: [quiet ]<n>s] · <detail>
#
...
# The `quiet ` marker is the pipeline's own, carried through verbatim: the run is
# still tracking that step, but its step log and native-agent lifecycle have been
# silent for longer than the pipeline's own quiet warning, so the age is real
# activity evidence and is never rendered as if it were fresh output.
...
if [ "$HAVE_RUN" = 1 ]; then
  ...
  # step, carrying its `quiet ` marker when the pipeline set one. Parsed from the
  # SAME $RUN_OUT already captured above, so this costs no extra query, and only
  # for a full `axi status` read: the coarse runs-list attribution carries no step
  # detail at all. A run that reports no active step or no parseable age simply
  # gets no field.
