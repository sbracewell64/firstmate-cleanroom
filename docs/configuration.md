...
Ordinary Desktop reconnect accepts an owned harness alongside its waiting shell regardless of process-list order, reusing the healthy primary without interrupting it or creating another workspace.
...
Before any console or doctor lifecycle work, the launcher asks the shared backend classifier whether its exact Herdr session-and-pane identity belongs to a recorded worker in this home, refuses a proven worker-owned pane without exposing task data, and fails closed when ownership cannot be verified.
Before a genuine startup wait, the launcher prints one short readiness progress line.
Concurrent Desktop launches serialize console creation, and unknown or conflicting existing ownership blocks another primary, including a direct console invocation without an existing matching session/pane record, without Herdr's injected socket identity for that session, or into a pane whose record has relinquished ownership.
...
The doctor's global-versus-effective scoped configuration reporting remains a separate limitation, not the reconnect cause or proof of a healthy scoped configuration.
...
Whatever a producer owns, the record itself is single-valued: `bin/fm-backlog-transition-lib.sh` owns what counts as a key answered twice and refuses such a record at the publication boundary every task-record writer crosses, so no writer can leave a second value shadowing the first.
`bin/fm-nm-observe.sh` owns the no-mistakes observation obligation at `state/<id>.nm-observe`, the durable per-task receipt at `data/<id>/nm-observation-receipt.md`, surviving exact-run ledgers at `data/nm-observation-runs/<run>.record`, per-run receipts at `data/<id>/nm-run-<run>-observation-receipt.md`, and the reconciliation cursor at `state/.nm-observe-watermark`; [`no-mistakes-observation.md`](no-mistakes-observation.md) owns the entrypoint census and coverage claims.
...
It also carries trusted `review.path_instructions`: a change to the programme-continuation resolver is reviewed as a complete machine-input boundary, and a change to the presentation owner is reviewed for presented-versus-acknowledged identity and its recovery and race cases.
The file's own comment records the pinned-runtime support evidence; the guidance is honored only from the default-branch copy and never replaces the colocated tests.
...
Launch mechanics, including caller-PATH executable selection for Codex workers and secondmates, missing-client refusal, raw-command ownership, and the verified command templates, live in [`bin/fm-spawn.sh`](../bin/fm-spawn.sh).
...
Use `bin/fm-work-context.sh discipline <id> ship implementation` to read the checked selection, `bin/fm-work-context.sh engineering <id> <worker|reviewer|all> <stage|all>` to read the complete checked context before applicable work and after resume, and `bin/fm-work-context.sh discipline-evidence <id> <run> <exact-head>` to integrity-check candidate evidence without changing qualification or landing.
...
The `worker-discipline` result in the shared `engineering-evidence.json` index binds discipline evidence to the current task, run, head, generation, selected level and fragment; its only outcomes are `OBSERVED` and `CNO`, both candidate evidence assessed by the existing validator rather than authority tokens.
...
It runs only when an independent backlog read-back confirms the child actually landed, and it fails closed rather than trusting a stale roadmap when the declared obligation is absent or carries a status that is neither `open` nor `landed` (only a genuinely landed matching line confirms idempotently).
...
FM_AUTOARM_RETIRE_POLLS=30   # tenths of a second the legacy auto-arm reclaim spends confirming a retired owner stopped; default 3s rather than the 1s it waited before, and floored at FM_STOP_REDELIVER_POLLS + 10 so the window always outlasts one re-delivery interval - raising that knob raises this wait with it
...
FM_PAUSE_RESURFACE_SECS=3600       # seconds before the watcher re-surfaces a declared external wait, a verified captain-held transfer, or an absorbed wedge deferral for a recheck, including a live busy pane past FM_BUSY_TURN_MAX_SECS; the away-mode daemon uses the same setting for a declared external wait or verified captain-held transfer, ageing its window against the crew's own latest status line rather than pane busy state
...
FM_PIPELINE_ACTIVITY_MAX_SECS=1800 # how long a crew whose lifecycle stage records a bound, running validation may go with no pipeline activity before its quiet pane returns to the wedge schedule; matches no-mistakes' own agent step timeout, and is deliberately generous because a legitimately quiet phase such as a long CI monitor can go many minutes between step log lines; an age no-mistakes has marked `quiet` is still the pipeline reporting the step it is tracking, so it is measured against this bound too rather than expiring at no-mistakes' own much shorter step_quiet_warning; a crew with no such declaration, no attributed run, or no readable activity keeps the unchanged schedule, and a value that is not a positive integer falls back to the default
