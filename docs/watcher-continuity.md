...
That shim never signals a live owner it cannot verify: one invocation records and delivers the only permitted TERM attempt to the exact proven-stuck owner whose recorded PID identity still matches, but retains its lock, later live observations deliver no second TERM, and only an invocation that freshly observes the owner gone may collect it; a TERM that was provably not delivered withdraws its unspent allowance so the owner stays retirable.
An owner whose recorded identity is absent or unreadable is never signalled at all, and that missing evidence never withholds the reclaim of a claim the ledger already proves abandoned.
[`turnend-guard.md`](turnend-guard.md) owns the surrounding guard behavior.
