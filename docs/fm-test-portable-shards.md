...
A local aggregate run without a manifest is refused when no lane file resolves at all, so it never publishes an empty report as a success; under a manifest that same absence is reported as every lane missing.
