...
- Slices A, B, and D bind no qualification record yet: their steps name the path where the qualification owner's record must land, and until it does the resolver reports `REQUIRED_BINDING_MISSING` for the first of them, never completion.
  Their accepted outcomes include the forge and ruling routes plus `DELIVERED_QUALIFIED` from the governed local-project delivery owner.
  A local delivery qualifies only after the programme pins its owner, record digest, generation, policy, exact candidate head/tree/delivery identity, and the resolver reproduces its private delivery receipt, distinct maker/checker identities, bound checker receipt, git candidate, digest-only privacy declaration, manifest, and destination read-back.
...
4. Qualification records for A, B, and D, owner the qualification and landing owners: when a slice is independently qualified, its owner record is authored at the step's `terminal_predicate.evidence` path as an `fm-accepted-owner-evidence/v1` record.
   ...
   The local route publishes identities and digests only; private candidate and checker receipts remain under the operational home, and an unavailable receipt or destination is CNO rather than qualification.
