...
state/               runtime records and signals; gitignored
  ...
  <id>.commit-identity  durable commit-identity obligation for a no-mistakes ship task (name=/email= lines); written by fm-spawn and consumed at the CI-ready boundary by bin/fm-pr-check.sh, which refuses to arm the merge poll for a pipeline commit that does not carry it (bin/fm-commit-identity-verify.sh owns the check)
