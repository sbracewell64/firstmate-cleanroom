...
The one exception is the wait it spends when the downtime marker lock is already held by someone else, which runs with the ordinary stop disposition still in force, because that wait has no deadline of its own and a holder that never releases would otherwise make the watcher unkillable.
A stop collected during that wait has mutated nothing, so it leaves the same held singleton lock and unpublished downtime as a watcher killed outright, which the next watcher's stale-lock steal publishes on its behalf.
