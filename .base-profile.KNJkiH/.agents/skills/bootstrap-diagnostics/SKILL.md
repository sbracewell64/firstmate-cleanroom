...
- `STARTUP_MEMORY_BUDGET: over budget - estimated <n> tokens exceed config/startup-memory-budget=<budget>; withheld from this digest: <files>; ...` (CONTEXT section) - the three memory files together exceed this home's allowance, so the digest injected only the whole files that fit in priority order and marked the rest `WITHHELD`.
...
- `STARTUP_MEMORY_BUDGET: memory not injected - <reason>; ...` (CONTEXT section) - the budget or one of the three memory files could not be validated as one set (an absent, malformed, or unsafe budget file, or a memory file that is unreadable or not an ordinary regular file), so all three were withheld rather than injected partially or stale.
