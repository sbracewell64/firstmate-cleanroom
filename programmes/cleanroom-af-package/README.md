...
- Slices A, B, and D bind no qualification record yet: their steps name the path where the qualification owner's record must land, and until it does the resolver reports `REQUIRED_BINDING_MISSING` for the first of them, never completion.
  ...
  D declares the complete seven-member S4 family, including the registry-named manager-loop plan, but pins no project; `--project auto` may select exactly one existing complete registered local-only owner, while no such owner remains CNO and creates no project or authority surface.
