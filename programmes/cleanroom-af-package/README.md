...
- Slices A, B, and D accept the forge and ruling routes plus `DELIVERED_QUALIFIED` from the governed local-project delivery owner.
  ...
  A sealed root the step does not admit is refused as `SOURCE_IDENTITY_MISMATCH` before any byte comparison, so a self-delivery whose source is its own destination can never qualify; the pins name no operator-local path.
  ...
  D declares the complete seven-member S4 family, including the registry-named manager-loop plan, and pins the unique complete `synthesis-work` owner selected by the full registered-project census.
