...
It evaluates every PR opening and body edit on that event's own snapshot, so a historically invalid body stays invalid even after a later clean edit.
On head synchronization or reopening it instead re-reads the live PR body and head, so a one-generation-late event payload no longer fails a PR whose live body is soundly attested to the live head, while a genuinely stale or absent live attestation still fails and an already-superseded head can never publish a result for a newer one.
It also prevents a later edit from replacing an earlier pending compliance check.
