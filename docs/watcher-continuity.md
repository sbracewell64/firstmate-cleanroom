...
That shim never collects a live owner without readable matching PID identity: one invocation records and delivers the only permitted TERM attempt to the exact proven-stuck owner but retains its lock, later live observations deliver no second TERM, and only an invocation that freshly observes the owner gone may collect it.
