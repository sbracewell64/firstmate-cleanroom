...
- Recovery of an existing presentation journal waits on the shared presentation lock for as long as a sibling spawn's whole reclaim, worktree, and launch window, then deliberately refuses the spawn if the lock is still contended rather than falling back flat, and default-on makes that refusal reachable in any Herdr home; the fresh projected-create path keeps its short wait, and `bin/fm-spawn.sh` owns both bounds.
...
Its before/after tripwire requires exactly one default session with a known running or stopped state and nonempty socket identity, and requires that snapshot to remain byte-identical.
