...
class GuardTests(unittest.TestCase):
    ...
    def test_exact_native_args(self):
        self.assertEqual(guard.launch_args([guard.POSTURE,'--model','gpt-5.6-luna']),['-c','forced_login_method="chatgpt"','-c','model_provider="openai"',guard.POSTURE,'--model','gpt-5.6-luna'])
    ...
    def test_effective_custom_provider_refuses(self):
        config={'model':'gpt-5.6-luna','forced_login_method':'chatgpt','model_provider':'openai','model_providers':{'openai':{'base_url':'https://private.invalid'}}}
        with self.assertRaises(guard.Refused):guard.check_config(config,'gpt-5.6-luna')
    def test_effective_root_endpoint_refuses(self):
        config={'model':'gpt-5.6-luna','forced_login_method':'chatgpt','model_provider':'openai','chatgpt_base_url':'https://private.invalid'}
        with self.assertRaises(guard.Refused):guard.check_config(config,'gpt-5.6-luna')
    def test_native_route_accepts_default_config(self):
        guard.check_config({'model':'gpt-5.6-luna','forced_login_method':'chatgpt','model_provider':'openai','model_providers':None},'gpt-5.6-luna')
