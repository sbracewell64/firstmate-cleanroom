...
The same library owns resolver stream capture for all five consumers: typed stdout and diagnostic stderr remain separate, diagnostics stay visible, and each executable's existing signal cleanup removes the shared staging record without a sourced-library trap replacing its caller's traps.
