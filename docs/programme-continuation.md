...
- Retirement survives later non-captain holds on the decision task: an external wait or a parked hold placed on it gates through the hold path, never re-fires the fact, and `--materialize` never replaces such a hold; only a live captain hold bound to that exact fact keeps it outstanding.
- Identity rule: `decision_key` is injective over required facts (captain_axes entries and enhancements with `required_to_proceed` true), so one required fact owns one key and one durable binding, a key shared by two required facts in one step or across steps is refused at load, and a non-required enhancement never becomes a fact or a task and so cannot overwrite, alias, or retire a required fact's binding by reusing its key.
