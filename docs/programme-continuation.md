...
Presentation and acknowledgement share the `.status-presentation-lock`, so an acknowledgement cannot promote a pending identity while a concurrent presenter is replacing that record.
...
`tests/fm-continuation-resolve.test.sh` also proves acknowledgement waits for the presentation lock before promoting its pending record, preserving the identity written by the presenter.
